...
void AFP_FirstPersonCharacter::Tick(float Deltaime)
{
	TraceForward();
}
...
void AFP_FirstPersonCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Interact", IE_Pressed, this, &AFP_FirstPersonCharacter::InteractPressed);
...
void AFP_FirstPersonCharacter::InteractPressed()
{
	TraceForward();
	if (FocusedActor) {
...
void AFP_FirstPersonCharacter::TraceForward_Implementation()
{
	APlayerController* PlayerController = Cast<APlayerController>(GetController());
